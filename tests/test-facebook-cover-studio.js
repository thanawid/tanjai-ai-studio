...
assert.match(app,/id="album-coverX"/);
assert.match(app,/id="album-coverY"/);
assert.match(app,/id="album-coverZoom"/);
assert.match(app,/data-photo-fit="fill"/);
assert.match(app,/data-photo-fit="contain"/);
assert.match(app,/id="album-bandEnabled"/);
assert.match(app,/id="album-bandColor"/);
assert.match(app,/id="album-bandOpacity"/);
assert.match(app,/id="album-bandStyle"/);
assert.match(app,/id="album-bandAccentColor"/);
assert.match(app,/id="album-bandAccentOpacity"/);
assert.match(app,/id="album-bandAccentWidth"/);
...
assert.match(app,/id="album-brandPhoneLabel"/);
...
assert.match(source,/function phoneText/);
...
assert.deepStrictEqual(Array.from(api.brandContacts({brandWebsite:'',brandSocial:'',org:'',brandPhone:'โทร. 02-000-0000',brandPhoneLabel:false}).map(item=>item.text)),['02-000-0000']);
assert.deepStrictEqual(Array.from(api.brandContacts({brandWebsite:'',brandSocial:'',org:'',brandPhone:'02-000-0000',brandPhoneLabel:true}).map(item=>item.text)),['โทร. 02-000-0000']);
assert.doesNotMatch(source,/detail\|\|'รายละเอียดว่าใครทำอะไร'/);

console.log(JSON.stringify({version:'12.6.8',coverModes:2,facebookFivePhotoPack:true,fullCoverDownload:true,freeCoverPosition:true,coverFitModes:2,editableBottomBand:true,goldBandDecor:true,optionalPhoneLabel:true,aiCaption:true,noPurposeField:true,officialBrandCover:true,status:'PASS'},null,2));
