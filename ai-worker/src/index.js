const TOOL_RULES = {
  ...
  post: "วิเคราะห์และสร้างสคริปต์วิดีโอ สคริปต์สรุปกิจกรรม สคริปต์เชิญชวน บทพากย์ เสียงตามสาย ข่าว โพสต์ หรือแคปชั่นตามชนิดผลงานที่เลือก",
  ...
  kit: "สร้างแกนการสื่อสารกลาง แล้วมอบหมายงานให้ผู้เชี่ยวชาญภาพ โพสต์ วิดีโอ เสียง และสไลด์โดยไม่คัดลอกข้อความเดียวกัน"
};

const SMART_FILL_POLICY = `นโยบายช่วยคิดอย่างปลอดภัย:
...
function buildPrompt(tool, data, options){
  ...
  if(tool === "post") return buildPostPrompt(data, options);
  return buildSpecialistPrompt(tool, data, options);
}

function buildSpecialistPrompt(tool, data = {}, options = {}){
  const roles = {
    image:{
      name:"ผู้กำกับศิลป์และนักออกแบบสื่อประชาสัมพันธ์",
      focus:"คิดลำดับสายตา องค์ประกอบ สี แสง ตัวแบบ พื้นที่ข้อความ ความอ่านง่ายบนมือถือ และ Prompt ผลิตภาพ",
      output:"Creative Direction, ข้อความบนภาพ, ลำดับข้อมูล, Prompt พร้อมสร้าง, Negative Prompt และรายการตรวจไฟล์จริง"
    },
    mc:{
      name:"นักเขียนบทพิธีกรและผู้กำกับเวที",
      focus:"คุมลำดับพิธี คำเรียกตำแหน่ง คำเชื่อม ภาษาพูด จังหวะหยุด และบัตรถืออ่าน โดยห้ามสลับกำหนดการเอง",
      output:"บทเปิด ลำดับพิธีพร้อมคำเชื่อม คำเชิญบุคคล ช่วงสำรอง บทปิด และบัตรย่อหน้างาน"
    },
    voice:{
      name:"นักเขียนบทเสียงและผู้กำกับการอ่านภาษาไทย",
      focus:"เขียนเพื่อการฟัง ไม่ใช่ภาษาเอกสาร คุมเวลาพูด การหายใจ จังหวะ คำเน้น อารมณ์ และคำอ่านชื่อเฉพาะ",
      output:"บทพร้อมอ่านจริง คำกำกับจังหวะเท่าที่จำเป็น คำเน้น คำอ่าน และเวลาประมาณการ"
    },
    deck:{
      name:"นักวางโครงเรื่องงานนำเสนอและบรรณาธิการสไลด์",
      focus:"สร้างเส้นเรื่องจากปัญหาไปสู่ข้อสรุป แยกข้อความบนสไลด์ออกจากคำพูด และไม่ยัดย่อหน้ายาวลงหน้า",
      output:"หัวข้อและวัตถุประสงค์ สไลด์ตามจำนวนที่เลือก เนื้อหาบนหน้า Visual Direction และ Speaker Notes ทุกหน้า"
    },
    kit:{
      name:"ผู้อำนวยการสื่อสารและหัวหน้าทีมผู้เชี่ยวชาญ",
      focus:"วาง Core Message และ Fact Register ชุดเดียว แล้วแตกงานให้ผู้กำกับภาพ นักประชาสัมพันธ์ ผู้กำกับวิดีโอ นักเขียนเสียง และนักวางสไลด์ทำหน้าที่ต่างกัน",
      output:"Campaign Spine, Fact Register, งานภาพ, โพสต์, วิดีโอ, เสียง, สไลด์ และรายการส่งมอบ โดยไม่คัดลอกข้อความเดียวกันทุกสื่อ"
    }
  };
  const role = roles[tool] || {name:"ผู้เชี่ยวชาญงานสื่อสาร",focus:TOOL_RULES[tool],output:"ผลงานพร้อมใช้ตามประเภทงาน"};
  const revision = options.revision
    ? `ปรับผลงานเดิมตามคำสั่ง “${options.revision}” โดยรักษาข้อเท็จจริงและโครงเฉพาะทาง:\n${data.originalText || ""}`
    : "สร้างฉบับแรกให้สมบูรณ์ แม้ข้อมูลสั้น โดยช่วยเติมเฉพาะเนื้อหาทั่วไปที่ปลอดภัย";
  return `คุณคือ${role.name}ของทันใจ AI Studio ไม่ใช่ผู้ช่วยทั่วไป

ความเชี่ยวชาญ: ${role.focus}
ภารกิจรอบนี้: ${revision}
...
function buildPostPrompt(data = {}, options = {}){
  const mode = options.channel || "ให้ AI วิเคราะห์และเลือกผลงาน";
  const modeRules = /ให้ AI วิเคราะห์/.test(mode)
    ? `วิเคราะห์ก่อนว่างานเกิดขึ้นแล้วหรือกำลังจะเกิด และเป้าหมายคือเชิญชวน แจ้งข่าว สรุปผล หรือให้ความรู้ จากนั้นเลือกชิ้นงานที่เหมาะที่สุด
ขึ้นต้นด้วย “ชิ้นงานที่เลือก: ...” เพียงบรรทัดเดียว แล้วส่งผลงานทันที ห้ามอธิบายกระบวนการคิด
ถ้าเป็นงานหลังจบกิจกรรมให้เลือกสคริปต์สรุปกิจกรรม ถ้าเป็นงานก่อนเกิดและต้องการคนเข้าร่วมให้เลือกสคริปต์เชิญชวน ถ้าต้องฟังโดยไม่เห็นภาพให้เลือกบทเสียง ถ้าเน้นอ่านให้เลือกข่าวหรือโพสต์`
    : /สคริปต์สรุปกิจกรรม/.test(mode)
      ? "เขียนสำหรับเหตุการณ์ที่เกิดขึ้นแล้ว แบ่งตามเวลา: เปิดเรื่อง, บรรยากาศ, การดำเนินงาน, ผลหรือประโยชน์ที่มีข้อมูลยืนยัน และปิดเรื่อง ทุกช่วงมีภาพที่เห็น บทพากย์ ข้อความบนจอ และเวลา ห้ามเขียนเป็นคำเชิญชวนก่อนงาน"
      : /สคริปต์เชิญชวน/.test(mode)
        ? "แบ่งตามเวลา: Hook ที่ตรงงาน, เหตุผลที่ผู้ฟังควรสนใจ, รายละเอียดวันเวลาและสถานที่ที่มีจริง, คำเชิญชวน ทุกช่วงมีภาพที่เห็น บทพากย์ ข้อความบนจอ และเวลา"
        : /สคริปต์วิดีโอ/.test(mode)
          ? "แบ่งตามเวลา: Hook, ภาพที่เห็น, บทพากย์, ข้อความบนจอ, ช่วงปิดและ CTA ให้เวลารวมตรงตามที่เลือก"
          : /เสียงตามสาย|รถประชาสัมพันธ์/.test(mode)
            ? "เขียนบทประกาศที่ฟังรู้เรื่องโดยไม่เห็นภาพ: คำเปิดเรียกความสนใจ, ใครจัด/เรื่องอะไร, วันเวลา/สถานที่, ทวนข้อมูลสำคัญ และคำปิด ใช้ประโยคสั้น อ่านชัด วนซ้ำได้ พร้อมคำอ่านชื่อเฉพาะ"
            : /บทพากย์|ทำเสียง/.test(mode)
              ? "ส่งบทพร้อมอ่านจริง มีคำกำกับจังหวะเท่าที่จำเป็น คุมเวลา และแยกคำอ่านชื่อเฉพาะ"
              : /ข่าวประชาสัมพันธ์/.test(mode)
                ? "เขียนพาดหัว โปรยข่าว เนื้อข่าวเรียงตามความสำคัญ และย่อหน้าปิด โดยไม่ใช้ภาษาขายเกินจริง"
                : /แคปชั่น YouTube|Reels|TikTok/.test(mode)
                  ? "ให้ชื่อคลิป คำโปรยสั้น คำอธิบายคลิป CTA และแฮชแท็กที่เกี่ยวข้องเท่านั้น"
                  : /ครบชุด/.test(mode)
                    ? "สร้าง 4 ส่วนที่ไม่คัดลอกกันตรง ๆ: สคริปต์วิดีโอ บทพากย์ โพสต์ Facebook และข้อความประกอบคลิป"
                    : "เขียนโพสต์ Facebook พร้อมเผยแพร่ มีหัวเปิด เนื้อหาอ่านง่าย CTA และแฮชแท็กตามตัวเลือก";
  const revisionRule = options.revision
    ? `นี่คืองานเดิมที่ต้องแก้:\n${data.originalText || options.originalText || ""}\n\nแก้เฉพาะตามคำสั่ง: ${options.revision} โดยห้ามเปลี่ยนข้อเท็จจริง`
    : "สร้างชิ้นงานใหม่จากข้อมูลของผู้ใช้";
  return `คุณคือกองบรรณาธิการประชาสัมพันธ์ภาษาไทยของทันใจ AI Studio มีความเชี่ยวชาญด้านสคริปต์วิดีโอ บทพากย์ ข่าวองค์กร และโซเชียลมีเดีย

ประเภทชิ้นงาน: ${mode}
ข้อกำหนดเฉพาะ: ${modeRules}
${revisionRule}
...
- แยกก่อนเขียนเป็น 2 ชั้น: “ข้อเท็จจริงเฉพาะงาน” ซึ่งห้ามเดา และ “ชั้นงานเขียน” ซึ่งต้องช่วยสร้างให้เต็มที่
- ชั้นงานเขียนที่ต้องช่วยสร้าง ได้แก่ มุมเล่า หัวเปิด ลำดับเรื่อง คำเชื่อม การอธิบายความหมาย ประโยชน์ทั่วไปที่สัมพันธ์กับหัวข้อ อารมณ์ น้ำเสียง บทสรุป และคำชวนลงมือทำแบบไม่อ้างช่องทางหรือเงื่อนไขที่ไม่มี
- หากเลือก “ช่วยคิดและแต่งให้สมบูรณ์” ให้ขยายข้อมูลสั้นเป็นผลงานเต็มฉบับที่อ่านลื่นและใช้ต่อได้
- หากเลือก “สร้างสรรค์มากขึ้น” ให้เพิ่มพลังของหัวเปิด ภาพจำ และวิธีเล่า โดยยังห้ามกล่าวเกินจริง
- หากเลือก “รักษาข้อมูลเป็นหลัก” ให้เกาะต้นฉบับมากที่สุด แต่ยังต้องเรียบเรียงภาษาและโครงสร้างให้ดี
- ห้ามใช้ความปลอดภัยเป็นเหตุผลส่งงานสั้น แห้ง หรือเพียงสรุปสิ่งที่ผู้ใช้กรอก
- ข้อมูลที่ยังขาดไม่ควรหยุดทั้งชิ้น ให้เขียนส่วนที่ทำได้ให้สมบูรณ์และระบุเฉพาะจุดสำคัญที่ต้องยืนยัน

กติกาบังคับ:
- รักษาข้อมูลใน lockedFacts ตามต้นฉบับทุกตัวอักษร ห้ามสลับวัน เวลา ตัวเลข ชื่อ และสถานที่
- ใช้ pronunciation กำกับเฉพาะงานที่ต้องอ่านออกเสียง
- ภาษาไทยต้องเป็นธรรมชาติ ไม่ขึ้นต้นและลงท้ายซ้ำแบบข้อความสำเร็จรูป
- ห้ามเขียนคำอธิบายตนเอง เช่น “จากข้อมูลที่ให้มา” “สารหลักคือ” หรือ “AI วิเคราะห์ว่า”
...
- ถ้าข้อมูลสำคัญขาด ให้ใส่ [กรุณาเติมข้อมูล] เฉพาะตำแหน่งนั้น ห้ามเดา
- ส่งเฉพาะผลงานพร้อมใช้ ไม่อธิบายวิธีคิด และไม่ใช้ Markdown code fence
