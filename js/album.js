/* V12.6.5 — Smart Facebook Post Studio
   ภาพปกสะอาดแบบงานจริง + AI เรียบเรียงแคปชั่นจากข้อเท็จจริง
...
(function(){
  ...
  const state={files:[],coverId:null,coverMode:'double',logo:null,logoUrl:'',outputs:[],caption:'',facts:{},headlineX:50,headlineY:58,logoX:4,logoY:5,drag:null,refreshTimer:null,refreshSeq:0};
  ...
  function ingestFiles(fileList){
    ...
    state.files=limited.map(makeFileItem);state.coverId=state.files[0]?.id||null;state.headlineX=50;state.headlineY=58;
  ...
  function drawCoverOverlays(ctx,w,h,d){
    const shade=ctx.createLinearGradient(0,0,0,h);if(d.coverStyle==='minimal'){shade.addColorStop(0,'rgba(0,0,0,.2)');shade.addColorStop(1,'rgba(0,0,0,.08)');}else if(d.coverStyle==='gradient'){shade.addColorStop(0,'rgba(0,0,0,.18)');shade.addColorStop(.42,'rgba(0,0,0,.08)');shade.addColorStop(1,'rgba(0,0,0,.72)');}else{shade.addColorStop(0,'rgba(0,0,0,.18)');shade.addColorStop(.42,'rgba(36,16,79,.06)');shade.addColorStop(1,'rgba(36,16,79,.78)');}ctx.fillStyle=shade;ctx.fillRect(0,0,w,h);
    if(state.logo&&d.logoPosition!=='none'){const box=w*(d.logoSize/100),x=w*(state.logoX/100),y=h*(state.logoY/100);ctx.save();ctx.shadowColor='rgba(0,0,0,.5)';ctx.shadowBlur=18;drawContained(ctx,state.logo,x,y,box,box);ctx.restore();}drawHeadline(ctx,w,h,d);
  }
  ...
  function useCoverTool(tool){
    ...
    if(tool==='reset'){state.headlineX=50;state.headlineY=58;state.logoX=formData().logoPosition==='right'?86:4;state.logoY=5;renderCoverEditor();scheduleGeneratedRefresh();notify('จัดตำแหน่งปกกลับค่าแนะนำแล้ว');}
  }
  ...
  window.TANJAI_ALBUM_PRO={generate,downloadAll,renderFacebookPreview,renderPhotoPicker,renderCropList,_test:{cropPlacement,captionWriter,factGuardCaption,short,metaText,headlineFontSize,normalizeFacts,cleanMultiline,wrapHeadlineLines}};
})();
