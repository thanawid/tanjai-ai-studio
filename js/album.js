/* V12.6.4 — Facebook Cover Studio
   ภาษาการออกแบบงานประชาสัมพันธ์ที่ยืดหยุ่น + ข้อมูลจริงชุดเดียวทุกผลลัพธ์
...
(function(){
  ...
  const state={files:[],coverId:null,coverMode:'double',logo:null,logoUrl:'',outputs:[],caption:'',facts:{},headlineX:50,headlineY:55,logoX:4,logoY:5,drag:null,refreshTimer:null,refreshSeq:0};
  ...
  function normalizeFacts(input={}){return{
    title:clean(input.title),org:clean(input.org),date:clean(input.date),time:clean(input.time),place:clean(input.place),detail:clean(input.detail),coverDetail:clean(input.coverDetail),purpose:clean(input.purpose),footer:clean(input.footer),captionStyle:clean(input.captionStyle)||'official'
  };}
  function formData(){
    const facts=normalizeFacts({title:val('album-title',''),org:val('album-orgName',''),date:val('album-dateTime',''),time:val('album-time',''),place:val('album-place',''),detail:val('album-detail',''),coverDetail:val('album-coverDetail',''),purpose:val('album-purpose',''),footer:val('album-footer',''),captionStyle:val('album-captionStyle','official')});
    state.facts=facts;
    return{...facts,coverStyle:val('album-coverStyle','editorial'),font:val('album-headlineFont','Kanit'),fontSize:num('album-headlineSize',72),fontWeight:num('album-headlineWeight',900),italic:checked('album-headlineItalic'),color:val('album-headlineColor','#f5c84c'),outlineColor:val('album-outlineColor','#24104f'),outlineWidth:num('album-outlineWidth',4),align:val('album-headlineAlign','center'),logoPosition:val('album-logoPosition','left'),logoSize:num('album-logoSize',10),bandColor:val('album-bandColor','#24104f'),bandOpacity:num('album-bandOpacity',82)/100,shadow:checked('album-headlineShadow'),divider:checked('album-dividerLine')};
  }
  ...
  function ingestFiles(fileList){
    ...
    state.files=limited.map(makeFileItem);state.coverId=state.files[0]?.id||null;state.headlineX=50;state.headlineY=55;
  ...
  function renderCoverEditor(){
    ...
    const isDouble=state.coverMode==='double',canvasW=isDouble?2160:1080,detail=coverDetailText(d),meta=metaText(d),headlineSize=headlineFontSize(d.title,d.fontSize,isDouble),headlineSourceSize=headlineSize*(isDouble?1.28:1),previewHeadlineSize=headlineSourceSize/canvasW*100,previewOutlineSize=d.outlineWidth*(isDouble?1.35:1)/canvasW*100,previewMetaSize=(isDouble?30:24)/canvasW*100,previewDetailSize=detailFontSize(detail,isDouble)/canvasW*100;
    const styleClass=`style-${d.coverStyle}`;
    ...
    const factsClass=meta&&detail?' has-both':meta?' meta-only':detail?' detail-only':' is-empty';
    host.innerHTML=`<div class="album-cover-stage ${styleClass}" id="albumCoverStage"><img class="album-editor-photo" src="${cover.url}" alt="ภาพปก" style="object-position:${cover.cropX}% ${cover.cropY}%"><div class="album-editor-shade"></div>${isDouble?'<i class="album-editor-seam" aria-hidden="true"></i>':''}${logoHtml}<div id="albumHeadlineDrag" class="album-headline-drag align-${d.align}${d.italic?' is-italic':''}" style="left:${state.headlineX}%;top:${state.headlineY}%;font-family:'${esc(d.font)}',sans-serif;font-size:${previewHeadlineSize}cqw;font-weight:${d.fontWeight};color:${d.color};-webkit-text-stroke:${previewOutlineSize}cqw ${d.outlineColor};text-shadow:${d.shadow?'0 3px 10px rgba(0,0,0,.72)':'none'}">${esc(d.title||'ข้อความพาดหัว')}</div><div class="album-editor-band" style="--album-band:${rgba(d.bandColor,d.bandOpacity)}"><i class="album-editor-divider${d.divider?'':' is-hidden'}"></i><div class="album-editor-facts${factsClass}">${meta?`<div class="album-editor-meta" style="font-size:${previewMetaSize}cqw">${esc(meta)}</div>`:''}${detail?`<div class="album-editor-detail" style="font-size:${previewDetailSize}cqw">${esc(detail)}</div>`:''}</div></div></div>`;
  ...
  function bindStageDrag(){
    ...
    [['#albumHeadlineDrag','headline'],['#albumLogoDrag','logo']].forEach(([selector,type])=>{const drag=$(selector);if(!drag)return;
  ...
  function drawHeadline(ctx,w,h,d){
    ...
    const maxWidth=w*.82,lines=wrapAllLines(ctx,text,maxWidth),lineH=size*1.13,x=w*(state.headlineX/100),centerY=h*(state.headlineY/100),startY=centerY-((lines.length-1)*lineH)/2;ctx.lineJoin='round';ctx.miterLimit=2;
  ...
  function drawBand(ctx,w,h,d){
    const bandH=d.coverStyle==='minimal'?150:250,y=h-bandH,meta=metaText(d),detail=coverDetailText(d),isDouble=w===2160;ctx.save();
    if(d.coverStyle==='gradient'){const gradient=ctx.createLinearGradient(0,y-120,0,h);gradient.addColorStop(0,'rgba(0,0,0,0)');gradient.addColorStop(.38,rgba(d.bandColor,d.bandOpacity*.55));gradient.addColorStop(1,rgba(d.bandColor,Math.min(1,d.bandOpacity+.1)));ctx.fillStyle=gradient;ctx.fillRect(0,y-120,w,bandH+120);}else{ctx.fillStyle=rgba(d.bandColor,d.coverStyle==='minimal'?d.bandOpacity*.55:d.bandOpacity);ctx.fillRect(0,y,w,bandH);}
    if(d.divider){const line=ctx.createLinearGradient(w*.1,0,w*.9,0);line.addColorStop(0,'rgba(255,255,255,0)');line.addColorStop(.12,'#f5c84c');line.addColorStop(.88,'#f5c84c');line.addColorStop(1,'rgba(255,255,255,0)');ctx.fillStyle=line;ctx.fillRect(w*.08,y+18,w*.84,5);}
    ctx.fillStyle='#fff';ctx.textBaseline='middle';
    if(isDouble){
      if(meta&&detail){ctx.font='700 30px "Sarabun","Noto Sans Thai",sans-serif';ctx.textAlign='center';const metaFit=fitDetailLines(ctx,meta,850,3,30,15),metaLineH=Math.round(metaFit.size*1.3),metaStart=y+138-((metaFit.lines.length-1)*metaLineH)/2;metaFit.lines.forEach((txt,i)=>ctx.fillText(txt,540,metaStart+i*metaLineH,850));const fitted=fitDetailLines(ctx,detail,920,7,detailFontSize(detail,true),10),lineH=Math.round(fitted.size*1.3),start=y+138-((fitted.lines.length-1)*lineH)/2;fitted.lines.forEach((txt,i)=>ctx.fillText(txt,1620,start+i*lineH,920));ctx.fillStyle='rgba(255,255,255,.25)';ctx.fillRect(1079,y+45,2,bandH-65);
      }else{const only=detail||meta;if(only){ctx.textAlign='center';const fitted=fitDetailLines(ctx,only,w*.82,6,detail?detailFontSize(detail,true):30,12),lineH=Math.round(fitted.size*1.32),start=y+138-((fitted.lines.length-1)*lineH)/2;fitted.lines.forEach((txt,i)=>ctx.fillText(txt,w/2,start+i*lineH,w*.82));}}
    }else{ctx.textAlign='left';if(meta){const metaFit=fitDetailLines(ctx,meta,972,2,24,14),metaLineH=Math.round(metaFit.size*1.3);metaFit.lines.forEach((txt,i)=>ctx.fillText(txt,54,y+58+i*metaLineH,972));}if(detail){const detailCenter=meta?y+155:y+138,fitted=fitDetailLines(ctx,detail,972,7,detailFontSize(detail,false),10),lineH=Math.round(fitted.size*1.28),start=detailCenter-((fitted.lines.length-1)*lineH)/2;fitted.lines.forEach((txt,i)=>ctx.fillText(txt,54,start+i*lineH,972));}}ctx.restore();
  }
  function drawCoverOverlays(ctx,w,h,d){
    ...
    if(state.logo&&d.logoPosition!=='none'){const box=w*(d.logoSize/100),x=w*(state.logoX/100),y=h*(state.logoY/100);ctx.save();ctx.shadowColor='rgba(0,0,0,.5)';ctx.shadowBlur=18;drawContained(ctx,state.logo,x,y,box,box);ctx.restore();}drawHeadline(ctx,w,h,d);drawBand(ctx,w,h,d);
  }
  ...
  window.TANJAI.applyAlbumTemplate=function(snapshot){Object.entries(snapshot||{}).forEach(([key,value])=>{const id=key.startsWith('album-')?key:`album-${key}`,el=document.getElementById(id);if(el&&typeof value!=='object')el.value=value;});if(snapshot?.coverMode)setCoverMode(snapshot.coverMode);if(Number.isFinite(snapshot?.headlineX))state.headlineX=snapshot.headlineX;if(Number.isFinite(snapshot?.headlineY))state.headlineY=snapshot.headlineY;if(Number.isFinite(snapshot?.logoX))state.logoX=snapshot.logoX;if(Number.isFinite(snapshot?.logoY))state.logoY=snapshot.logoY;renderCoverEditor();};
  window.TANJAI_ALBUM_PRO={generate,downloadAll,renderFacebookPreview,renderPhotoPicker,renderCropList,_test:{cropPlacement,captionWriter,factGuardCaption,short,metaText,coverDetailText,detailFontSize,headlineFontSize,normalizeFacts}};
})();
