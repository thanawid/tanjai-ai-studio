...
// Render forms
  ...
  $("#imageForm").innerHTML = TANJAI.field("image") + `
    <div class="form-section simple-smart-section-v98 creative-quality-wrap-v91">
      <div class="section-title"><b>2</b><h4>เลือกแนวภาพหลัก</h4></div>
      <p class="mini-note">เลือกแค่แนวภาพที่ต้องการ ระบบจะเลือก AI เฉพาะทางหลังบ้านให้อัตโนมัติ ไม่ต้องให้ผู้ใช้ตัดสินใจเอง</p>
      <div class="form-grid">
        <select id="image-specialist" hidden aria-hidden="true" data-proofread-skip="1">${opts(toolOptions.imageSpecialists)}</select>
...
$("#mcResult").innerHTML = TANJAI.readyOutputShell("mc", "สคริปต์พิธีกรพร้อมใช้ — MC Writer", "คุมลำดับพิธี ชื่อ ตำแหน่ง คำเชื่อม และบัตรเตือนพิธีกร", "mcOut");
  ...
  const syncImageSpecialist = (forceToast=false) => {
    ...
    const contentType = $("#image-contentType")?.value || "";
    const mainCategory = $("#image-mainCategory")?.value || "";
    const subCategory = $("#image-subCategory")?.value || "";
    const title = $("#image-title")?.value || "";
    const detail = $("#image-detail")?.value || "";
    const text = `${visual} ${ctx} ${imageType} ${contentType} ${mainCategory} ${subCategory} ${title} ${detail}`;
    let role = "AI Poster Creative Director";
    if(/สรุปกิจกรรม|ข่าวประชาสัมพันธ์|recap|Photo Grid|หลายภาพ|ลงพื้นที่|ภาพกิจกรรม|รายงานผล/i.test(text)) role = "AI Event Recap Designer";
    else if(/การศึกษา|วิทยาลัย|โรงเรียน|ศูนย์พัฒนาเด็กเล็ก|นักเรียน|นักศึกษา|ครู|อบรมเด็ก/i.test(text)) role = "AI Education Poster Designer";
    else if(/อินโฟกราฟิก|Infographic|ขั้นตอน|ข้อมูลควรรู้|วิธีใช้งาน|FAQ|ตัวเลข|สถิติ/i.test(text)) role = "AI Infographic Planner";
    else if(/การ์ตูน|มาสคอต|Character|คาแรกเตอร์|2D|3D/i.test(text)) role = "AI Character Visual Designer";
    else if(/เพลง|Cover|Cinematic|หนัง|แฟนตาซี|ศิลปิน|ปกเพลง/i.test(text)) role = "AI Cinematic Cover Director";
    else if(/ประกาศ|แจ้งข่าว|announcement|เตือน|รับสมัคร|เปิดลงทะเบียน/i.test(text)) role = "AI PR News Designer";
    setSelectOptionValue("#image-specialist", role);
    specialistEl.dataset.autoRole = role;
    if(forceToast) TANJAI.toast("ระบบเลือก AI เฉพาะทางให้อัตโนมัติแล้ว");
  };
