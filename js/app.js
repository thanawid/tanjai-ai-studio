...
$("#mcResult").innerHTML = TANJAI.readyOutputShell("mc", "สคริปต์พิธีกรพร้อมใช้ — MC Writer", "คุมลำดับพิธี ชื่อ ตำแหน่ง คำเชื่อม และบัตรเตือนพิธีกร", "mcOut");
  ...
  // Firebase Auth Integration — username-only gate
  (function(){
    ...
    if(loginForm){
      loginForm.addEventListener("submit", async e => {
        ...
        setAuthBusy(true);
        try {
          if(typeof window.TANJAI_AUTH.loginName === "function"){
            await window.TANJAI_AUTH.loginName(name);
          } else {
            ...
            await window.TANJAI_AUTH.login(profile.email, "");
          }
