...
<div class="section-title"><b>4</b><h4>การปกป้องภาพจริง</h4></div>
  ...
  // Results
  $("#imageResult").innerHTML = TANJAI.resultShell("image", "คำสั่งพร้อมใช้", "ใช้กับทันใจ GPT / Canva / AI สร้างภาพ", "imageOut", `<button class="btn secondary" data-image-viewmode="prompt">Prompt</button><button class="btn secondary" data-image-viewmode="gpt">พร้อมใช้</button><button class="btn primary" data-copy-image="prompt">คัดลอก Prompt</button><button class="btn secondary" data-copy-image="gpt">คัดลอกพร้อมใช้</button>`);
$("#postResult").innerHTML = TANJAI.resultShell("post", "สคริปต์สรุปงาน", "ใช้ต่อได้ทั้งนำเสนอ ทำคลิป และโพสต์โซเชียล", "postOut", `<button class="btn primary" data-copybox="postOut">คัดลอกโพสต์</button>`);
  ...
  TANJAI.updateImageResultMode = function(mode){
    const titleEl = $("#imageResultTitle");
    const descEl = $("#imageResultDesc");
    const outEl = $("#imageOut");
    if(!titleEl || !descEl || !outEl) return;
    if(mode === "prompt") {
      titleEl.textContent = "Prompt";
      descEl.textContent = "ใช้ต่อกับ AI สร้างภาพภายนอก";
      outEl.textContent = TANJAI.state.lastImagePrompt || outEl.textContent;
      TANJAI.state.lastImageMode = "prompt";
      return;
    }
    titleEl.textContent = "คำสั่งพร้อมใช้";
    descEl.textContent = "คัดลอกแล้วเปิดทันใจ GPT ต่อได้ทันที";
    outEl.textContent = TANJAI.state.lastImageGPT || outEl.textContent;
    TANJAI.state.lastImageMode = "gpt";
  };


  // V8.2 Command Palette + FAB Quick Menu
  TANJAI.quickActions = [
  ...
  TANJAI.mountV82Navigation = function(){
    ...
    wrap.innerHTML = `
      ...
      <div id="fabMenu" class="fab-menu" hidden>
        <button data-fab-view="image">🖼️ สร้างภาพ</button>
        <button data-fab-view="post">✍️ โพสต์</button>
        <button data-fab-view="video">🎬 วิดีโอ</button>
        <button data-fab-view="voice">🎙️ เสียง</button>
        <button data-fab-view="deck">📊 สไลด์</button>
        <button data-fab-open="gpt">🤖 เปิด GPT</button>
      </div>
    ...
    document.body.addEventListener("click", (e)=>{
      ...
      if(fabOpen){ window.open(TANJAI.normalizeGPTUrl(TANJAI_CUSTOM_GPT_URL), "_blank", "noopener,noreferrer"); fabMenu.hidden = true; return; }
    });
