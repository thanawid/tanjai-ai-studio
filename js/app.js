...
<div class="section-title"><b>4</b><h4>การปกป้องภาพจริง</h4></div>
  ...
  // Results
  $("#imageResult").innerHTML = TANJAI.resultShell("image", "คำสั่งพร้อมใช้สำหรับทันใจ GPT", "พร้อมคัดลอกไปทำงานต่อกับทันใจ GPT หรือสลับดู Prompt สำหรับสร้างภาพได้ทันที", "imageOut", `<button class="btn secondary" data-image-viewmode="prompt">ดู Prompt</button><button class="btn secondary" data-image-viewmode="gpt">ดูคำสั่งพร้อมใช้</button><button class="btn primary" data-copy-image="prompt">คัดลอก Prompt</button><button class="btn secondary" data-copy-image="gpt">คัดลอกคำสั่งพร้อมใช้</button>`);
$("#postResult").innerHTML = TANJAI.resultShell("post", "สคริปต์สรุปงาน", "ใช้ต่อได้ทั้งนำเสนอ ทำคลิป และโพสต์โซเชียล", "postOut", `<button class="btn primary" data-copybox="postOut">คัดลอกโพสต์</button>`);
  ...
  TANJAI.updateImageResultMode = function(mode){
    ...
    if(mode === "prompt") {
      titleEl.textContent = "Prompt สำหรับสร้างภาพ";
      descEl.textContent = "คัดลอก Prompt ไปใช้ต่อกับ AI สร้างภาพภายนอกได้ทันที";
      outEl.textContent = TANJAI.state.lastImagePrompt || outEl.textContent;
      TANJAI.state.lastImageMode = "prompt";
      return;
    }
    titleEl.textContent = "คำสั่งพร้อมใช้สำหรับทันใจ GPT";
    descEl.textContent = "คัดลอกแล้วเปิดทันใจ GPT ต่อได้ทันที พร้อมคำสั่งปลายทางให้สร้างภาพจริง";
