...
$("#mcResult").innerHTML = TANJAI.readyOutputShell("mc", "สคริปต์พิธีกรพร้อมใช้ — MC Writer", "คุมลำดับพิธี ชื่อ ตำแหน่ง คำเชื่อม และบัตรเตือนพิธีกร", "mcOut");
  ...
  (function(){
    ...
    // รายชื่อที่ใช้งานได้ → แปลงเป็น email อัตโนมัติ
    const DOMAIN = "@tanjai.local";
    const VALID_USERS = ["thanawid", "teamtanjai"];

    function resolveEmail(input) {
      const raw = (input || "").trim().toLowerCase();
      // ถ้าพิมพ์ email เต็มอยู่แล้ว ใช้เลย
      if (raw.includes("@")) return raw;
      // ถ้าพิมพ์แค่ชื่อ ต่อ domain ให้
      return raw + DOMAIN;
    }
    ...
    if(loginForm){
      loginForm.addEventListener("submit", async e => {
        ...
        // ตรวจ username ที่อนุญาต (ถ้าไม่ใช่ email เต็ม)
        if(!raw.includes("@")) {
          const uname = raw.toLowerCase();
          if(!VALID_USERS.includes(uname)){
