...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const toolOptions = {
    ...
    workTypes: ["ประชาสัมพันธ์กิจกรรม","สรุปผลงาน / รายงาน","แจ้งข่าวสาร / ประกาศ","เชิญชวนเข้าร่วม","รณรงค์ / สร้างความตระหนัก","โปรโมทสินค้า / บริการ","แนะนำโครงการ / แผนงาน","อบรม / สัมมนา / ประชุม","งานพิธีกร / เปิดงาน","ขอบคุณ / แสดงความยินดี","อื่น ๆ"],
...
<div class="section-title"><b>4</b><h4>การปกป้องภาพจริง</h4></div>
  ...
  // Override label ของ image-title ตรงๆ ไม่ผ่าน ui.js (กัน cache)
  (function fixImageLabel(){
    const lbl = document.querySelector('label[for="image-title"], #imageForm label.full:first-child');
    const inp = document.getElementById("image-title");
    if(inp){
      // ใส่ label ใหม่ตรงๆ ผ่าน closest label
      const parentLabel = inp.closest("label");
      if(parentLabel){
        const firstNode = parentLabel.childNodes[0];
        if(firstNode && firstNode.nodeType === 3){
          firstNode.textContent = "ข้อความหลักบนภาพ / ชื่องาน";
        }
        // update hint
        const hint = parentLabel.querySelector(".field-hint");
        if(hint) hint.textContent = "(ข้อความนี้จะขึ้นบนภาพจริง — ใส่ชื่อจริง ไม่ใช่ประเภทงาน)";
        // update placeholder
        inp.placeholder = "เช่น  ชื่อเพลง / ชื่อกิจกรรม / หัวข้อประกาศ / ชื่อโปรเจกต์ ที่ต้องการขึ้นบนภาพ";
      }
    }
  })();
  $("#albumForm").innerHTML = `
  ...
  $("#mcForm").innerHTML = TANJAI.field("mc") + `
    ...
    <div class="form-section"><div class="section-title"><b>2</b><h4>ตั้งค่างานพิธีกร</h4></div>
      <div class="form-grid">
        <label>ประเภทงาน<select id="mc-workType"><option>งานพิธีกร / ผู้ดำเนินรายการ</option><option>พิธีเปิด / พิธีปิด</option><option>งานประชุม / อบรม</option><option>งานมอบเกียรติบัตร / มอบรางวัล</option><option>งานชุมชน / งานองค์กร</option><option>งานทางการ</option><option>อื่น ๆ</option></select></label>
...
- เหมาะกับงานที่ต้องปล่อยหลายสื่อพร้อมกัน`
  ...
  $("#makeImage").onclick = () => {
    ...
    TANJAI.toast("สร้าง Prompt เรียบร้อย — คัดลอกไปวางใน ทันใจ GPT ได้เลย");
