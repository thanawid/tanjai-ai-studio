...
$("#mcResult").innerHTML = TANJAI.readyOutputShell("mc", "สคริปต์พิธีกรพร้อมใช้ — MC Writer", "คุมลำดับพิธี ชื่อ ตำแหน่ง คำเชื่อม และบัตรเตือนพิธีกร", "mcOut");
  ...
  // Firebase Auth Integration — username + password gate
  (function(){
    ...
    if(signUpBtn){
      ...
      signUpBtn.parentElement?.classList.add("single-action");
    }
    ...
    const setAuthBusy = (busy) => {
      if(signInBtn){
        ...
        signInBtn.textContent = busy ? "กำลังเข้าสู่ระบบ…" : "เข้าสู่ระบบ";
      }
    };
    ...
    if(loginForm){
      loginForm.addEventListener("submit", async e => {
        ...
        if(!name){
          if(loginError) loginError.textContent = "กรุณากรอกชื่อผู้ใช้งาน";
          return;
        }
        if(!password){
          if(loginError) loginError.textContent = "กรุณากรอกรหัสผ่าน";
          return;
        }
        ...
        setAuthBusy(true);
        try {
          if(typeof window.TANJAI_AUTH.loginWithUsername === "function"){
            await window.TANJAI_AUTH.loginWithUsername(name, password);
          } else {
            const users = window.TANJAI_USERNAME_USERS || {};
            const key = name.toLowerCase().replace(/[^a-z0-9._-]/g, "");
            const profile = users[key];
            const email = name.includes("@") ? name : profile?.email;
            if(!email) throw new Error("ไม่พบชื่อผู้ใช้งานนี้ในระบบ");
            await window.TANJAI_AUTH.login(email, password);
          }
          if(loginEmail && window.TANJAI_AUTH.getCurrentUser){
            loginEmail.value = window.TANJAI_AUTH.getCurrentUser()?.email || "";
          }
        } catch (error) {
          if(loginError) loginError.textContent = error.message || "เข้าสู่ระบบไม่สำเร็จ";
        } finally {
          setAuthBusy(false);
        }
      });
    ...
    logoutBtn?.addEventListener("click", () => window.TANJAI_AUTH?.logout?.());
  })();
