  ...
  $("#videoForm").innerHTML = TANJAI.field("video") + `
    ...
    <div class="form-section"><div class="section-title"><b>2</b><h4>ตั้งค่า Tanjai Video Studio</h4></div>
      <div class="form-grid">
        ...
        <label>เสียงใน CapCut<select id="video-voiceMode">${opts(toolOptions.videoVoiceModes)}</select><small>เลือก “บทพูดตัวละคร” เมื่อต้องการให้คนในเรื่องพูด ไม่ใช่เสียงผู้บรรยาย</small></label>
...
- เหมาะกับงานที่ต้องปล่อยหลายสื่อพร้อมกัน`
  ...
  $("#makeVideo").onclick = async () => {
    ...
    Object.assign(d, {length, format, channel, aspectRatio, videoStyle, outputPack, voiceMode, videoVoiceMode: voiceMode});
    const team=TANJAI.freeWritingTeam;
    const aiResult=await TANJAI.generateWritingWithAI({
      tool:"video", data:d,
      options:{length, format, channel, aspectRatio, videoStyle, outputPack, voiceMode},
    ...
    TANJAI.setReadyOutput("video", {
      ...
      advanced2:TANJAI.executionPrompt("video", d, {length, format, channel, aspectRatio, videoStyle, outputPack, voiceMode})
    });
