  ...
  (function(){
    ...
    const normalizeLoginIdentifier = value => {
      ...
      if(!/^[a-z0-9._-]+$/.test(raw)){
        throw new Error("ชื่อผู้ใช้ใช้ได้เฉพาะ a-z, 0-9, จุด ขีดกลาง หรือขีดล่าง");
      }
      return `${raw}@tanjai.local`;
    };
    TANJAI.normalizeLoginIdentifier = normalizeLoginIdentifier;
    ...
    if(loginForm){
      loginForm.addEventListener("submit", async e => {
        ...
        try { email = normalizeLoginIdentifier(loginEmail.value); }
        catch(error){ if(loginError) loginError.textContent = error.message; return; }
        if(!email || !password){
          if(loginError) loginError.textContent = "กรุณากรอกชื่อผู้ใช้หรืออีเมล และรหัสผ่าน";
          return;
        }
    ...
    if(signUpBtn && allowRegistration){
      signUpBtn.addEventListener("click", async () => {
        ...
        if (!email || !password) {
          if(loginError) loginError.textContent = "กรุณากรอกชื่อผู้ใช้หรืออีเมล และรหัสผ่านเพื่อสมัครสมาชิก";
          return;
        }
    ...
    [logoutBtn, sidebarLogoutBtn].filter(Boolean).forEach(button => {
      button.addEventListener("click", async () => {
        button.disabled = true;
        if (window.TANJAI_AUTH) {
          try { await window.TANJAI_AUTH.logout(); }
          finally {
