...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const toolOptions = {
    ...
    videoFormats: ["คลิปประชาสัมพันธ์","มิวสิกวิดีโอ / MV เพลง","วิดีโอเนื้อเพลง","นิทานเด็ก","หนังสั้น","ซีรีส์สั้น","โฆษณาสินค้า","คลิปรีวิวสินค้า","สารคดีสั้น","การ์ตูน / แอนิเมชัน","คลิปสั้นแนวตั้งสำหรับ TikTok / Reels / Shorts","คลิปข่าวด่วน","คลิปกิจกรรม / โครงการ","คลิปโซเชียลไวรัล"],
    videoAspectRatios: ["แนวตั้ง 9:16 — TikTok / Reels / Shorts","แนวนอน 16:9 — YouTube / Facebook / จอทีวี","จัตุรัส 1:1 — Feed / Ads","แนวตั้ง 4:5 — Facebook / Instagram Feed","ให้ AI เลือกตามช่องทาง"],
    videoStyles: ["ให้ AI เลือกตามงาน","ภาพยนตร์ อารมณ์ลึก ซีนสวย","โซเชียลตัดไว ทันสมัย","เล่าเรื่องอบอุ่น ดูง่าย","สารคดีภาพจริง น่าเชื่อถือ","สดใสสำหรับเด็กและครอบครัว","โฆษณาสินค้าสะอาด มืออาชีพ","มิวสิกวิดีโอ เน้นอารมณ์เพลง","วิดีโอเนื้อเพลง อ่านชัด","การ์ตูน / แอนิเมชัน เล่าเรื่องด้วยตัวละคร"],
    videoOutputPacks: ["Prompt รายช็อต + เสียง CapCut + บทพูดตัวละคร (แนะนำ)","แพ็กผลิตวิดีโอครบชุด","Storyboard + บทบรรยาย","Prompt วิดีโอ AI รายช็อต","แผนตัดต่อใน CapCut","แพ็ก MV / วิดีโอเนื้อเพลง","แพ็กโฆษณาสั้น"],
    videoVoiceModes: ["บรรยาย + บทพูดตัวละคร (แนะนำ)","เฉพาะบทพูดตัวละคร","เฉพาะเสียงบรรยาย","เฉพาะเนื้อเพลง"],
    videoDestinations: ["ให้ AI เลือกตามงาน","CapCut","Google Veo / Flow","Runway","Luma","HeyGen","Pika","Kling / Seedance"],
...
// Render forms
  ...
  $("#imageForm").innerHTML = TANJAI.field("image") + `
    ...
    <select id="image-specialist" hidden>
      <option selected>ให้ AI เลือกตามงาน</option>
      <option>AI Creative Director</option>
      <option>Thai PR Premium Specialist</option>
      <option>Infographic Specialist</option>
      <option>Photo-Safe Retouch Specialist</option>
    </select>
...
$("#albumForm").innerHTML = `
  ...
  $("#postForm").innerHTML = TANJAI.field("post") + `
    ...
    <div class="form-section post-writer-section"><div class="section-title"><b>2</b><h4>เลือกงานที่จะเขียน</h4></div>
      ...
      <div class="form-grid post-writer-options">
        ...
        <label>งานนี้ต้องการสื่ออะไร<select id="post-purpose"><option selected>ให้ AI วิเคราะห์จากข้อมูล</option><option>เชิญชวนก่อนจัดงาน</option><option>สรุปกิจกรรมที่ดำเนินงานแล้ว</option><option>แจ้งข่าวหรือประกาศ</option><option>ให้ความรู้หรือรณรงค์</option><option>รายงานความคืบหน้าหรือผลการดำเนินงาน</option><option>สร้างการรับรู้ทั่วไป</option></select></label>
  ...
  $("#videoForm").innerHTML = TANJAI.field("video") + `
    <div class="form-note">Tanjai Video Studio ใช้วางแพ็กผลิตวิดีโอพร้อมทำต่อ: Hook, Storyboard, Voice Over, On-screen Text, Shot List, AI Video Prompt และแผนตัดต่อ</div>
    <div class="form-section"><div class="section-title"><b>2</b><h4>ตั้งค่า Tanjai Video Studio</h4></div>
      <div class="form-grid">
        ...
        <label>สัดส่วนภาพ<select id="video-aspectRatio">${opts(toolOptions.videoAspectRatios)}</select></label>
        <label>สไตล์วิดีโอ<select id="video-style">${opts(toolOptions.videoStyles)}</select></label>
        <label>ปลายทาง AI วิดีโอ<select id="video-destination">${opts(toolOptions.videoDestinations)}</select><small>เลือกเครื่องมือที่จะเอา prompt ไปใช้ เช่น Veo รวมภาพ+เสียง, Runway เน้นภาพ, HeyGen เน้นคนพูด</small></label>
        <label>โหมดเสียง / บทพูด<select id="video-voiceMode">${opts(toolOptions.videoVoiceModes)}</select><small>เลือก “บทพูดตัวละคร” เมื่อต้องการให้คนในเรื่องพูด ไม่ใช่เสียงผู้บรรยาย</small></label>
        <label class="full">ชุดผลลัพธ์<select id="video-outputPack">${opts(toolOptions.videoOutputPacks)}</select></label>
      </div>
    </div><div class="button-row"><button class="btn primary" id="makeVideo">✨ สร้าง Video Production Pack</button><button class="btn secondary" id="saveVideo">บันทึก</button></div>`;
...
$("#postResult").innerHTML = TANJAI.readyOutputShell("post", "ผู้ช่วยเขียนงานประชาสัมพันธ์", "เลือกประเภทงาน วางข้อมูลจริง แล้วระบบจะสร้างฉบับพร้อมใช้และแยกจุดที่ต้องตรวจสอบ", "postOut") + `
  ...
  if(postStatusLabel) postStatusLabel.textContent = "ผลงานพร้อมใช้";
$("#mcResult").innerHTML = TANJAI.readyOutputShell("mc", "สคริปต์พิธีกรพร้อมใช้ — MC Writer", "คุมลำดับพิธี ชื่อ ตำแหน่ง คำเชื่อม และบัตรเตือนพิธีกร", "mcOut");
  ...
  TANJAI.setPostWriterMode = function(mode){
    ...
    const revisionSets={
      video:new Set(["shorter","natural","hook","thirtySeconds","threeMinutes","toVoice","toCaption","proofread"]),
      voice:new Set(["shorter","formal","natural","hook","thirtySeconds","toCaption","proofread"]),
      social:new Set(["shorter","formal","natural","hook","expand","variant","proofread"]),
      news:new Set(["shorter","formal","natural","hook","expand","variant","toCaption","proofread"]),
      complete:new Set(["shorter","formal","natural","proofread"])
    };
    const group=/สคริปต์วิดีโอ/.test(mode) ? "video" : /บทพากย์|เสียงตามสาย|รถประชาสัมพันธ์/.test(mode) ? "voice" : /ข่าวประชาสัมพันธ์/.test(mode) ? "news" : /ครบชุด/.test(mode) ? "complete" : "social";
    document.querySelectorAll("[data-post-revise]").forEach(button=>{button.hidden=!revisionSets[group].has(button.dataset.postRevise);});
  };
  ...
  // Generators
  const syncImageSpecialist = (notify=false) => {
    const specialist = $("#image-specialist");
    if(!specialist) return;
    const visual = $("#image-visualPreset")?.value || "";
    const imageType = $("#image-imageType")?.value || "";
    const editMode = $("#image-editMode")?.value || "";
    let next = "AI Creative Director";
    if(/Thai PR Premium|ประชาสัมพันธ์ราชการ|โปสเตอร์ประชาสัมพันธ์/.test(`${visual} ${imageType}`)) next = "Thai PR Premium Specialist";
    if(/Infographic|อินโฟกราฟิก|ข้อมูล/.test(`${visual} ${imageType}`)) next = "Infographic Specialist";
    if(/ภาพจริง|ปรับภาพ|รีทัช|ต้นฉบับ/.test(`${visual} ${imageType} ${editMode}`)) next = "Photo-Safe Retouch Specialist";
    specialist.value = next;
    if(notify) TANJAI.toast?.("อัปเดตผู้เชี่ยวชาญภาพแล้ว");
  };
...
- เหมาะกับงานที่ต้องปล่อยหลายสื่อพร้อมกัน`
  ...
  TANJAI.generateCurrentImage = async function(button){
    try{
      if(!window.TANJAI_AI_CONFIG?.imageGenerationEnabled){
        TANJAI.toast?.("ปิดปุ่มสร้างภาพในเว็บไว้ก่อน — ใช้ Prompt ภาพต่อได้ทันที");
        return;
      }
      if(!TANJAI.state.lastImageGPT || !String(TANJAI.state.lastImageGPT).trim()){
        $("#makeImage")?.click();
      }
      const prompt = TANJAI.state.lastImageGPT || $("#imageOut")?.textContent || "";
      if(!String(prompt).trim()){
        TANJAI.toast?.("กรุณากดสร้าง Prompt ภาพก่อน");
        return;
      }
      const d = getImageDataForPrompt();
      d.prompt = prompt;
      const result = await TANJAI.generateImageWithAI({
        prompt,
        data:d,
        options:{size:$("#image-size")?.value || d.size || "", visualPreset:$("#image-visualPreset")?.value || d.visualPreset || ""},
        fallback:()=>prompt,
        button
      });
      if(result.imageUrl){
        TANJAI.state.lastImageUrl = result.imageUrl;
        TANJAI.showImageResult?.(result.imageUrl, {prompt, model:result.model});
        window.TANJAI_AUTH?.trackUsage("image_generate");
        return;
      }
      TANJAI.setReadyOutput("image", {
        title:"ยังสร้างภาพในเว็บไม่ได้ — ใช้ Prompt นี้ต่อได้",
        desc:"ระบบหลังบ้านยังไม่พร้อมสร้างภาพ หรือ AI Image ตอบกลับไม่สำเร็จ แต่ Prompt ยังพร้อมนำไปใช้กับ ทันใจ GPT / Canva / เครื่องมือภาพอื่น",
        main:result.prompt || prompt,
        advancedTitle1:"สาเหตุจาก AI Image",
        advanced1:result.error || "ยังไม่ได้เชื่อม AI Image API"
      });
    }catch(error){
      console.error("TANJAI_GENERATE_IMAGE_ERROR", error);
      TANJAI.toast?.("สร้างภาพในเว็บไม่สำเร็จ — ใช้ Prompt ภาพแทนก่อน");
    }
  };
  ...
  $("#makePost").onclick = async () => {
    ...
    TANJAI.toast("สร้างงานเขียนพร้อมใช้ให้แล้ว");
  ...
  document.querySelectorAll("[data-post-revise]").forEach(button => {
    button.addEventListener("click", async () => {
      ...
      const aiResult=await TANJAI.generateWritingWithAI({
        ...
        options:{...originalOptions, revision:revisionLabels[revision], preserveFacts:true, silentStatus:true},
  ...
  $("#makeVideo").onclick = async () => {
    ...
    const channel=$("#video-channel")?.value || "";
    const aspectRatio=$("#video-aspectRatio")?.value || "";
    const videoStyle=$("#video-style")?.value || "";
    const outputPack=$("#video-outputPack")?.value || "";
    const voiceMode=$("#video-voiceMode")?.value || "";
    const destination=$("#video-destination")?.value || "";
    Object.assign(d, {length, format, channel, aspectRatio, videoStyle, outputPack, voiceMode, videoVoiceMode: voiceMode, destination, videoDestination: destination});
    const team=TANJAI.freeWritingTeam;
    const aiResult=await TANJAI.generateWritingWithAI({
      tool:"video", data:d,
      options:{length, format, channel, aspectRatio, videoStyle, outputPack, voiceMode, destination},
      fallback:()=>team.videoWriter(d, length), button:$("#makeVideo")
    });
    const executeOut=aiResult.text;
    TANJAI.setReadyOutput("video", {
      title:"Video Production Pack — Tanjai Video Studio",
      desc:aiResult.source === "ai" ? "AI จัดแพ็กวิดีโอพร้อมผลิตจากบรีฟแล้ว" : "แพ็กวิดีโอจากระบบสำรอง พร้อมส่งต่อ CapCut, ทีมตัดต่อ หรือ AI Video",
      main:executeOut,
