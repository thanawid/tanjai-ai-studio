...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const toolOptions = {
    ...
    videoFormats: ["คลิปประชาสัมพันธ์","คลิปข่าวด่วน","คลิปกิจกรรม / โครงการ","คลิปรีวิว","คลิปโซเชียลไวรัล","คลิปแนวสารคดีสั้น","TikTok (9:16 · 15–60 วินาที)","Instagram Reels (9:16 · 15–90 วินาที)","Facebook Reels (9:16 · 15–60 วินาที)","YouTube Shorts (9:16 · สูงสุด 60 วินาที)","Line VOOM Short"],
...
// Render forms
  ...
  $("#imageForm").innerHTML = TANJAI.field("image") + `
    <div class="form-section creative-quality-wrap-v91"><div class="section-title"><b>2</b><h4>บริบทงาน + Creative Quality</h4></div>
      <p class="mini-note">ไม่ต้องตั้งค่าก็ได้ ระบบจะทำหน้าที่เป็น PR Creative Director เลือกคอนเซปต์ โทนภาพ และ Layout ตามบรีฟให้อัตโนมัติ โดยไม่แต่งข้อมูลจริง</p>
      <div class="form-grid">
...
<div class="section-title"><b>4</b><h4>การปกป้องภาพจริง</h4></div>
    ...
    <div class="button-row image-btn-row">
      <button class="btn primary" id="makeImage">📋 สร้าง Prompt</button>
      <button class="btn secondary" id="saveImage">บันทึก</button>
    </div>
    <p class="btn-flow-hint">กด "สร้าง Prompt" → คัดลอกไปวางใน ChatGPT <span class="btn-flow-sep">·</span> หรือกด "สร้างภาพ AI" หลังสร้าง Prompt แล้ว</p>
  `;
...
$("#mcResult").innerHTML = TANJAI.readyOutputShell("mc", "สคริปต์พิธีกรพร้อมใช้ — MC Writer", "คุมลำดับพิธี ชื่อ ตำแหน่ง คำเชื่อม และบัตรเตือนพิธีกร", "mcOut");
  ...
  // Firebase Auth Integration v9.5 — Username Login
  (function(){
    ...
    // ตาราง username → email จริง (แต่ละคนใช้ email ต่างกัน)
    const USER_MAP = {
      "thanawid":   "thanawid@gmail.com",
      "teamtanjai": "teamtanjai@tanjai.local"
    };

    function resolveEmail(input) {
      const raw = (input || "").trim().toLowerCase();
      // ถ้าพิมพ์ email เต็มอยู่แล้ว ใช้เลย
      if (raw.includes("@")) return raw;
      // ค้นจากตาราง
      return USER_MAP[raw] || null;
    }

    if(signUpBtn) signUpBtn.hidden = !allowRegistration;
    ...
    if(loginForm){
      loginForm.addEventListener("submit", async e => {
        ...
        // ตรวจ username — ต้องอยู่ในตาราง USER_MAP
        if(!raw.includes("@") && !resolveEmail(raw)){
          if(loginError) loginError.textContent = `ไม่พบชื่อผู้ใช้ "${raw}" ในระบบ กรุณาตรวจสอบอีกครั้ง`;
          return;
        }

        const email = resolveEmail(raw) || raw;
    ...
    if(signUpBtn && allowRegistration){
      signUpBtn.addEventListener("click", async () => {
        ...
        if(loginError) loginError.textContent = "";
        if (!raw || !password) {
          if(loginError) loginError.textContent = "กรุณากรอกชื่อผู้ใช้และรหัสผ่านเพื่อสมัครสมาชิก";
          return;
        }
        if (confirm(`สมัครสมาชิกด้วยชื่อ "${raw}" ใช่หรือไม่?`)) {
          if(!window.TANJAI_AUTH){
            if(loginError) loginError.textContent = "ระบบสมาชิกยังไม่พร้อม กรุณาตรวจสอบอินเทอร์เน็ตแล้วลองใหม่";
            return;
          }
    ...
    // ปุ่มออกจากระบบใน sidebar (มือถือ)
    ...
    // expose albumState for adjust engine
    window.albumState = window.albumState || {};
  })();
...
- เหมาะกับงานที่ต้องปล่อยหลายสื่อพร้อมกัน`
  ...
  $("#makeImage").onclick = () => {
    ...
    TANJAI.state.lastImageData = d;
    TANJAI.updateImageResultMode?.("gpt");
    TANJAI.toast("สร้าง Prompt สำเร็จ — กดสร้างภาพเลยได้ด้านล่าง");
    window.TANJAI_AUTH?.trackUsage("image");
    TANJAI.showPollinationsBtn();
  };

  /* ─── POLLINATIONS.AI — สร้างภาพฟรีในเว็บ ─── */
  TANJAI.showPollinationsBtn = function() {
    const host = $("#imageResult");
    if(!host) return;
    if($("#pollinationsSection")) $("#pollinationsSection").remove();
    const sec = document.createElement("div");
    sec.id = "pollinationsSection";
    sec.className = "pollinations-section";
    sec.innerHTML = `
      <div class="pollinations-header">
        <span>🎨</span>
        <div><b>สร้างภาพในเว็บเลย</b><small>Pollinations.ai — ฟรี ไม่ต้อง API key</small></div>
      </div>
      <div class="pollinations-controls">
        <select id="pollinationsSize">
          <option value="1080x1080">1:1 Facebook / IG</option>
          <option value="1080x1350" selected>4:5 Facebook โพสต์</option>
          <option value="1080x1920">9:16 Reels / Story</option>
          <option value="1920x1080">16:9 YouTube</option>
        </select>
        <button class="btn primary" id="generatePollinationsBtn">✨ สร้างภาพเลย</button>
      </div>
      <div id="pollinationsOutput" style="display:none;margin-top:12px">
        <div id="pollinationsLoading" style="display:flex;align-items:center;gap:10px;padding:12px;background:var(--panel);border-radius:10px">
          <div class="pollinations-spinner"></div>
          <span style="font-size:13px;color:var(--muted)">กำลังสร้างภาพ... (15–30 วินาที)</span>
        </div>
        <img id="pollinationsImg" style="display:none;width:100%;border-radius:12px;margin-top:10px" alt="ภาพที่สร้างโดย AI">
        <div id="pollinationsActions" class="pollinations-actions" style="display:none">
          <button class="btn primary" id="pollinationsDownload">⬇ ดาวน์โหลด</button>
          <button class="btn secondary" id="pollinationsRegen">🔄 สร้างใหม่</button>
        </div>
        <p style="font-size:11px;color:var(--muted);margin-top:8px">⚠️ AI อาจสะกดข้อความผิด — ตรวจสอบก่อนนำไปใช้จริง</p>
      </div>`;
    host.appendChild(sec);

    const generate = async () => {
      const d = TANJAI.state.lastImageData || {};
      const size = $("#pollinationsSize")?.value || "1080x1350";
      const [w,h] = size.split("x").map(Number);
      const title  = (d.title || "professional poster").substring(0,80);
      const org    = (d.orgName || "").substring(0,40);
      const color  = (d.colorTone || "purple and gold").substring(0,40);
      const style  = (d.style || "modern clean").substring(0,40);
      const engPrompt = [
        `professional Thai PR poster, ${style}`,
        `main text: "${title}"`,
        org ? `brand: ${org}` : "",
        `color scheme: ${color}`,
        "bold Thai typography, clean layout, high quality, sharp text"
      ].filter(Boolean).join(", ");
      const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(engPrompt)}?width=${w}&height=${h}&nologo=true&seed=${Date.now()}&enhance=true`;
      $("#pollinationsOutput").style.display="block";
      $("#pollinationsLoading").style.display="flex";
      $("#pollinationsImg").style.display="none";
      $("#pollinationsActions").style.display="none";
      $("#generatePollinationsBtn").disabled=true;
      const img=$("#pollinationsImg");
      img.onload=()=>{
        $("#pollinationsLoading").style.display="none";
        img.style.display="block";
        $("#pollinationsActions").style.display="flex";
        $("#generatePollinationsBtn").disabled=false;
        $("#pollinationsDownload").onclick=()=>{ const a=document.createElement("a");a.href=url;a.download="tanjai-image.jpg";a.target="_blank";a.click(); };
      };
      img.onerror=()=>{
        $("#pollinationsLoading").innerHTML="<span style='color:var(--muted)'>⚠️ สร้างไม่สำเร็จ กรุณาลองใหม่</span>";
        $("#generatePollinationsBtn").disabled=false;
      };
      img.src=url;
    };
    setTimeout(()=>{
      $("#generatePollinationsBtn")?.addEventListener("click",generate);
      $("#pollinationsRegen")?.addEventListener("click",generate);
    },100);
  };
