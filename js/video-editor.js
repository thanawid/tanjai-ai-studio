...
(() => {
  ...
  function renderStories(){const sec=$('#storyOptionsSection'),voice=$('#voiceScriptSection'),selected=$('#selectedClipsSection'),exp=$('#editorExportBox');if(!state.analysis){sec.hidden=voice.hidden=selected.hidden=exp.hidden=true;const lp=$('#videoLookPanel');if(lp)lp.hidden=true;return;}sec.hidden=voice.hidden=selected.hidden=exp.hidden=false;const lp=$('#videoLookPanel');if(lp)lp.hidden=false;$('#storyOptionGrid').innerHTML=state.storyOptions.map((s,i)=>`<button type="button" class="story-option ${i===state.selectedStory?'active':''}" data-story="${i}"><small>${s.badge}</small><b>${s.title}</b><span>${fmtTime(s.duration)} • ${s.focus}</span><ol>${s.structure.map(x=>`<li>${esc(x)}</li>`).join('')}</ol></button>`).join('');$$('[data-story]').forEach(b=>b.onclick=()=>selectStory(Number(b.dataset.story)));$('#voiceScriptText').value=state.voiceScript;}
