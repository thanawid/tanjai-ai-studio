...
STOP RULE: เรียกเครื่องมือสร้างภาพตอนนี้ และอย่าส่งคำอธิบายก่อนภาพ`;
  ...
  T.buildImageExecutionPrompt = function(d = {}){
    ...
    if(realPhotoMode){
      ...
      direction = [
        "Modern Vivid Anime Comic Illustration — ลายเส้นอนิเมะคอมิกร่วมสมัยแบบงานวาดมืออาชีพ เส้นหมึกดำหนา คม สะอาด มีน้ำหนักเส้นชัดและรูปทรงเฉียบ",
        "ลงสีแบบ polished cel shading แบ่งแสงเงาเป็นระนาบคม ใช้สีอิ่มสด คอนทราสต์สูง ไฮไลต์ชัด และฉากมีมิติ ห้ามออกเป็นพาสเทลนุ่ม สีน้ำ ภาพแบน หรือภาพกึ่งสมจริง",
        "ตัวละครมีสีหน้าและท่าทางมีพลังแบบ manga/comic proportions รายละเอียดผม เสื้อผ้า และวัตถุคมชัด แต่ไม่เลียนแบบตัวละครหรือศิลปินที่มีลิขสิทธิ์",
        "ให้ประเภทภาพและ Layout ที่ผู้ใช้เลือกเป็นผู้กำหนดการจัดหน้า: ถ้าเป็นอินโฟกราฟิกจึงแบ่งบล็อกข้อมูลได้ แต่ถ้าเป็นโปสเตอร์หรือภาพประกอบ ห้ามบังคับแบ่งช่อง กรอบคำพูด หรือเส้นพุ่ง"
      ];
      if(infographic){
        direction.push("จัดข้อมูลเป็นบล็อกหรือช่องที่อ่านตามลำดับได้ชัด โดยคงลายเส้น สี และ cel shading ชุดเดียวกันทั้งภาพ");
      }
    }else if(infographic){
      direction = [
...
G. CapCut Character Dialogue Clean Script โดยครอบด้วย [CAPCUT_CHARACTER_DIALOGUE] และ [/CAPCUT_CHARACTER_DIALOGUE] เท่านั้นในส่วนนั้น
H. Shot List แยก Must-have / Nice-to-have / B-roll สำรอง
I. Editing Notes: subtitle, safe area, rhythm, color mood, transition, caption style
J. AI Video Prompt Pack รายช็อตแบบละเอียด สำหรับนำไปใช้กับเครื่องมือสร้างวิดีโอ
K. Fact/Asset Checklist ก่อนผลิต
...
${silentQualityGate([
  ...
  "CapCut Character Dialogue แยกจากบทบรรยายและไม่มีชื่อตัวละครในบล็อกคัดลอก",
...
ห้ามแต่งข้อเท็จจริง และไม่ต้องแสดงกระบวนการคิดภายใน`;
  ...
  const quickLabels = {
    ...
    post:{title:"เรื่องที่ต้องการสื่อสาร", detail:"วางข้อมูลจริงทั้งหมดที่มี ไม่ต้องเรียงประโยค ระบบจะวิเคราะห์และเขียนให้"},
